...
const optionalDriveLinkField = document.getElementById('optionalDriveLink');
...
function handleSubmitButtonClicked(event) {
    ...
    let labObject = {
        ...
        TAemail: auth.currentUser.email,
        labName: labNameField.value ,
        password: passwordField.value,
        driveLink: optionalDriveLinkField.value
    }
    ...
    fetch("/create", {
        ...
        if (response.ok){
            ...
            alert('error creating the given lab, try another name');
        }
    })
